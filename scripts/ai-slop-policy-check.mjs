...
/**
 * AI instruction policy check
 *
 * Validates instruction quality using deterministic checks:
 * - instruction budget checks (lines / hard rules / examples)
 * - effective instruction discovery
 * - cross-file language and execution conflict checks
 */
...
function isScopedInstructionFile(rootDir, filePath) {
  ...
  if (SCOPED_INSTRUCTION_RELATIVE_PATTERNS.some((pattern) => pattern.test(normalizedRelativePath)))
    return true
...
function checkInstructionBudgets(rootDir, files, failures) {
  for (const filePath of files) {
    ...
    if (lineCount > INSTRUCTION_LINE_LIMIT) {
      failures.push(
        `${relativePath} -> exceeds instruction line budget (${lineCount} > ${INSTRUCTION_LINE_LIMIT}).`,
      )
    }
    ...
    if (exampleBlockCount > exampleBlockLimit) {
      failures.push(
        `${relativePath} -> exceeds example block budget (${exampleBlockCount} > ${exampleBlockLimit}).`,
      )
    }
  }
}
