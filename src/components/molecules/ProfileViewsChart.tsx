...
import type { DashboardReportingSnapshot } from "@/lib/clinic-dashboard/reporting"
...
export function ProfileViewsChart({ chart }: ProfileViewsChartProps) {
  const gradientId = useId().replaceAll(":", "")
  const pointRefs = useRef<Array<SVGGElement | null>>([])
  const [activeIndex, setActiveIndex] = useState<number | null>(null)
  const [tabIndex, setTabIndex] = useState(0)
  const chartMaximum = Math.max(...chart.points.map((point) => point.value)) * 1.1
  const coordinates = chart.points.map((point, index, points) => {
  ...
  const chartLine = coordinates.map(({ x, y }) => `${x},${y}`).join(" ")
  ...
  return (
    <div className="max-w-full overflow-x-auto pb-2">
      ...
      >
        ...
        <polygon fill={`url(#${gradientId})`} points={chartArea} />
        <polyline
          fill="none"
          points={chartLine}
          stroke="var(--primary)"
