...
The capability list contains each value exactly once in the order shown. During the initial treatment rollout, profile
view and edit access respectively inherit treatment view and edit access. The list is a UI projection, not a
replacement for Payload authorization. Each read or mutation still authorizes the current principal, clinic, document,
and fields.
