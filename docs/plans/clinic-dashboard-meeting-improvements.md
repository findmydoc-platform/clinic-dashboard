...
Clinic staff can navigate a clearer dashboard prototype, understand reporting relationships, recognize the active clinic location as part of the clinic identity, use safer prototype-backed profile and review workflows, and see future product areas without mistaking them for completed capabilities.
...
| 3. Static clinic location identity              | The active location reads as part of the clinic identity without adding a separate header control.                              | Show `Berlin Health Clinic — Mitte` as one static header identity in both prototype modes and retain `Mitte, Berlin` in the dashboard location summary. Render no location selector, alternate location fixture, location capability, or local selection state. Do not emit a clinic or tenant ID.                                                                                                                                                                                                                                    | None.                                        |
...
| 6. Moderated review responses                   | A clinic response is submitted for moderation instead of appearing to publish directly.                                         | Keep dialog text as a local draft until submit. A valid submit creates a separate `pending-moderation` response and never changes it into a published response. When no published response exists, the card shows only the pending state; when editing a published response, keep the published text visible and show the pending edit separately. Do not set the review to `Answered` until a published response exists, and never reuse appeal status for response moderation. Add no retry or withdrawal action.                   | None.                                        |
| 7. Appeal case audit trail                      | Appeal references and state changes belong to an appeal case and can be followed over time.                                     | Replace the free review notice with an optional typed appeal case containing a deterministic reference, reason, ISO timestamped events, and the closed prototype status set `submitted` or `under-review`. Creation starts at `submitted` and appends `appeal-submitted`; the only allowed prototype transition is `submitted` → `under-review`, which appends `appeal-status-changed`. Render events oldest to newest in a semantic ordered list, with `No appeal case has been opened` when absent. Keep case data management-only. | Slice 6.                                     |
...
Every slice 1–11 must complete the paired-record review in the same work item:
...
3. Verify the matching Website Capability Matrix row when one exists and record the unchanged contract or explicit no-contract result in the slice pull request. Do not edit the website repository merely because a matching row exists.

If the backend owner, status, public visibility, cache impact, or capability classification has drifted, stop that slice and obtain explicit cross-repository scope before changing the website matrix. Any companion website issue, branch, review, or pull request is outside these eleven Clinic Dashboard slices until separately approved. The initial matrix review above does not waive the per-slice verification requirement.
...
- Treat `visual-reference` as the complete prototype inventory. `presentation` hides support, Subscriptions, and Certificates and accreditations because no approved backend contract owns them. The clinic identity remains static in both modes.
...
- Browser Storybook tests own isolated component interaction, dialogs, keyboard and focus behavior, accessible names, state communication, responsive 320 px layouts, and theme variants. Slices 2, 4, 5, 6, 7, 9, 10, and 11 add explicit dark stories with `globals: { theme: "dark" }`.
- New end-to-end assertions are limited to the missing-treatment action opening the honest support prototype. The existing authentication, navigation, sign-out, and theme smoke coverage remains.
...
| 3     | Given either prototype mode, when the workspace renders, then the header shows `Berlin Health Clinic — Mitte`, the dashboard summary shows `Mitte, Berlin`, and no location selector or alternate identity is present. At 320 px, the static identity does not cause horizontal overflow.        | No separate location policy remains.                                                                                                                    | Static identity in both modes, selector absence, and 320 px layout.                                                         | None.                                                                 | Light screenshot confirms the location is attached to the clinic name.              |
| 4     | Given an unused catalogue entry, when it is selected, then the clinic relationship uses its master ID and a price. Existing entries permit price edits only, duplicates fail, duration is absent, and the missing-treatment action opens support only in `visual-reference`.                     | Catalogue/relationship mapping, duplicate rejection, price update, remove/undo, and absence of duration.                                                | Add/edit/remove flows, focus return, presentation gating, support handoff, and explicit dark variant.                       | Missing treatment opens the support prototype and no request is sent. | Light and dark dialog screenshots.                                                  |
...
| 6     | Given a review with or without a published response, when valid draft text is submitted, then a separate pending response appears; published text remains unchanged, and review/appeal statuses do not impersonate moderation. No retry or withdrawal control is present.                        | Draft-to-pending transition, published preservation, status invariants, empty validation, and deterministic timestamps.                                 | New response, edited response, focus return, pending label, prohibited-action absence, and explicit dark variant.           | None.                                                                 | Light and dark screenshots.                                                         |
| 7     | Given no appeal case, history shows the explicit empty state. When an appeal is submitted, then one `submitted` case and `appeal-submitted` event exist; changing it to `under-review` appends one event, and no other transition or final decision is available.                                | Closed status set, allowed transition, reference ownership, unique event IDs, ISO timestamps, chronological order, and no case data on the review card. | Empty, submitted, under-review, focus behavior, presentation hiding, prohibited final decisions, and explicit dark variant. | None.                                                                 | Light and dark screenshots; final reviews Product Design audit.                     |
...
pnpm deadcode:check
pnpm deps:audit
python3 -m pip install semgrep==1.159.0
...
Reviewers are read-only. Before each reviewer run, the coordinator recommends the named reviewers and obtains user confirmation. Each implementation pull request receives at most one consolidated review run; the listed roles may participate in that run, but reviewer-driven fixes are validated locally and do not trigger a second reviewer pass. After the run, every finding is presented before any reviewer-driven fix is applied; fixes require separate user confirmation.
