    ...
    clinic-dashboard/
      ...
      clinic-profile/
        ...
        components/
          ...
          organisms/
        hooks/
          useClinicProfileSourceController.ts
        model/
        server/
          public.ts
        testing/
...
| Server entry                   | central data-provider composition, private workspace provider, workspace input, demo loader, and narrow domain or aggregate `server/public.ts` contracts | concrete domain providers, components, browser adapters, demo client commands, fixtures               |
...
| Workspace                      | feature `public.ts` contracts, workspace model, shared UI                                                                                                | raw demo data, other private feature leaf components, test fixtures, live-domain providers            |
...
A controller is a named React hook or a thin component that connects state, commands, effects, and a view model. Examples include `useReviewsController` and `useClinicProfileSourceController`.
...
Feature mutation boundaries are narrow domain contracts such as `ClinicProfileSourceCommands`, `ClinicGalleryCommands`, `ClinicTreatmentCommands`, `ReviewSourceCommands`, and `InquiryStatusCommands`. Their browser adapters call the approved Dashboard BFF routes; the central server composition selects Controlled or Payload providers. The browser never receives Payload credentials or private transport details. `ClinicDashboardDemoInteractionPolicy` controls only presentation behavior that is still synthetic. It must never represent or replace Payload authorization capabilities such as `clinic-profile:view` and `clinic-profile:edit`. Support request state deliberately remains local-only while no approved external support capability exists. A future named support adapter and command contract require separate approval and are not preimplemented.
...
Feature mutations use feature-owned browser adapters that call the Dashboard origin. The server-side data-provider composition selects Controlled or Payload implementations before feature controllers receive their narrow command contracts; no parallel client demo profile adapter exists. Public profile completion is evaluated from the clinic-bound profile, gallery, and treatment sources during server workspace assembly through server-safe feature entries. The serialized result is atomic, independent of reporting periods, and never recalculated from client-side profile projections. Its percentage reflects all readable sources, while its task list includes only destinations backed by the authenticated clinic user's matching edit capability.

The only private cross-area exceptions are: `server.ts` importing the central data-provider composition, demo workspace provider, workspace contract, and live-domain server contracts; the central composition importing exact live-domain contracts and concrete providers; exact provider contract and adapter tests importing those private modules; approved doctor, inquiry, and review BFF routes importing the root server entry; `demo/loader.ts` implementing the workspace provider; demo builders importing the private workspace input type; and the existing `PrototypeModeSwitch` composition. All other cross-area imports use the owning area's `public.ts`.

Screens receive view models. Controllers receive the smallest command contract they need. The production page starts directly in the polished `presentation` mode and does not render a mode switch. `visual-reference` and the switch remain internal Storybook/QA tools. Only notification read IDs may survive in `sessionStorage`. Profile, gallery, treatment, inquiry-status, and review workflows use website-owned Payload contracts in normal mode; Controlled providers keep synthetic mutations in process-local state and reset on server restart. Timeline system events are intentionally session-local. Message and support demo changes reset on reload or location change. Further Payload integrations and Dashboard-owned persistence remain out of scope until separately planned.
